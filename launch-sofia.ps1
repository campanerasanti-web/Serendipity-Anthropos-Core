# MASTER LAUNCH SCRIPT - El Mediador de Sofia (Ruta absoluta)
# EJECUTAR: .\launch-sofia.ps1
...
$buildOutput = & $dotnetPath build 2>&1
...
$backendScript = {
    ...
    & $dotnetExe run --urls "http://0.0.0.0:5000" 2>&1
}
...
if ($backendRunning -and $backendRunning.State -eq "Running") {
    ...
    $log = Receive-Job -Job $backendJob -ErrorAction SilentlyContinue
    Write-Host $log
    exit 1
}
...
Write-Host ""
Write-Host "SISTEMA OPERATIVO - TODO LANZADO OK" -ForegroundColor Magenta
Write-Host "Tiempo: $([int]$elapsedTime.TotalSeconds)s" -ForegroundColor Cyan
Write-Host ""
Write-Host "Nada me pertenece. Todo es del Padre. El punto de anclaje esta establecido." -ForegroundColor Yellow
Write-Host ""

# Monitoreo
Write-Host "Presiona Ctrl+C para detener servicios" -ForegroundColor Gray
Write-Host ""
